...
if new_analysis:
    ...
    if uploaded_file_simulation_db:        
        ...
        if uploaded_file:
            ...
            df5 = load_data(uploaded_file)  # Load the Process Mapping data
            ...
            if isinstance(df5, dict):
                with processmapping_col1:
                    # Allow user to select a sheet
                    sheet_name = st.selectbox("Select the sheet", df5.keys())

                if sheet_name in df5:
                    selected_data = df5[sheet_name]
                ...
                with rtv_col:
                    ...
                    # Display results
                    st.text_input('Wt per board (including wastage %)', value=f"{wt_per_board_incl_wastage:.2f}", key="wt_per_board_incl_wastage", disabled=True)
                ...
                with solder_top_col:
                    ...
                    # Top weight estimate percentage and solder paste wastage percentage in the same line
                    weight_estimate_col1, weight_estimate_col2 = st.columns(2)
                    with weight_estimate_col1:
                        top_weight_estimate_percentage = st.text_input('Top Weight Estimate %', value="", key="top_weight_estimate_percentage")
                    with weight_estimate_col2:
                        top_sp_wastage_percentage = st.text_input('Top Solder Paste Wastage %', value="", key="top_sp_wastage_percentage")

                    # Paste specific gravity and cost of solder bar in the same line
                    specific_gravity_col1, specific_gravity_col2 = st.columns(2)
                    with specific_gravity_col1:
                        paste_specific_gravity = st.text_input('Paste Specific Gravity(g/cc)', value="7.31", key="paste_specific_gravity", disabled=True)
                    with specific_gravity_col2:
                        cost_of_solder_bar = st.text_input('Cost of Solder Bar($/g)', value="0.065", key="cost_of_solder_bar", disabled=True)

                    # Other inputs
                    solder_paste_thickness = st.text_input('Solder Paste Thickness(mm)', value="", key="solder_paste_thickness")
                    # weight_of_solder_paste_for_100percentage_wt = st.text_input('Weight of solder paste for 100%(g)', value="0.0", key="weight_of_solder_paste_for_100percentage_wt", disabled=True)
                    ...
                    # top_side_cost_per_board = st.text_input('Top Side Cost Per Board($)', value="0.0", key="top_side_cost_per_board", disabled=True)
                    ...
                    try:
                        ...
                        cost_of_solder_bar = float(cost_of_solder_bar) if cost_of_solder_bar else 0.065
                    ...
                    # Display results
                    st.text_input('Weight of solder paste for 100%(g)', value=f"{weight_of_solder_paste_for_100percentage_wt_value:.2f}", key="weight_of_solder_paste_for_100percentage_wt", disabled=True)
                    st.text_input('Top Weight of solder paste for Weight Estimate (g)', value=f"{top_weight_of_solder_paste_for_wt_estimate_value:.2f}", key="top_solder_paste_weight_estimate", disabled=True)
                    st.text_input('Top Side Cost Per Board($)', value=f"{top_side_cost_per_board_value:.2f}", key="top_side_cost_per_board", disabled=True)                # Solder Paste - Bottom
                ...
                with solder_bottom_col:
                    ...
                    # Input Fields
                    bot_weight_estimate_percentage = st.text_input('Bottom Weight Estimate %', value="", key="bot_weight_estimate_percentage")
                    bot_sp_wastage_percentage = st.text_input('Bottom Solder Paste Wastage %', value="", key="bot_sp_wastage_percentage")
                    # bot_solder_paste_weight_estimate = st.text_input('Weight of solder paste for Weight Estimate (g)', value="0.0", key="bot_solder_paste_weight_estimate", disabled=True)
                    # bot_side_cost_per_board = st.text_input('Bottom Side Cost Per Board($)', value="0.0", key="bot_side_cost_per_board", disabled=True)
                    ...
                    # Display results
                    st.text_input('Bottom Weight of solder paste for Weight Estimate (g)', value=f"{bot_weight_of_solder_paste_for_wt_estimate_value:.2f}", key="bot_solder_paste_weight_estimate", disabled=True)
                    st.text_input('Bottom Side Cost Per Board($)', value=f"{bot_side_cost_per_board_value:.2f}", key="bot_side_cost_per_board", disabled=True)
                ...
                with flux_col:
                    ...
                    # flux_board_area = st.text_input('Board Area(mm^2)', value="0.0", key="flux_board_area", disabled=True)
                    # flux_spread_area = st.text_input('Flux Spread Area(mm^2)', value="0.0", key="flux_spread_area", disabled=True)
                    ...
                    # Display results
                    st.text_input('Board Area(mm^2)', value=f"{flux_board_area_value:.2f}", key="flux_board_area", disabled=True)
                    ...
                    flux_cost_per_board = st.text_input('Flux Cost Per Board($)', value="0.0", key="flux_cost_per_board", disabled=True)
                ...
                with input_cost_col:
                    ...
                    cost_nre = st.text_input('NRE ($)', value="", key="cost_nre")
                ...
                with placeholder2_col:
                    ...
                    if st.button("Save Consumable, RM & Conversion Costing Details"):
                        if sheet_name in st.session_state.edited_sheets:
                            ...
                            current_data.loc[0, "Top Weight Estimate %"] = top_weight_estimate_percentage
                            current_data.loc[0, "Top Solder Paste Wastage %"] = top_sp_wastage_percentage
                            current_data.loc[0, "Paste Specific Gravity(g/cc)"] = paste_specific_gravity
                            current_data.loc[0, "Cost of Solder Bar($/g)"] = cost_of_solder_bar
                            current_data.loc[0, "Solder Paste Thickness(mm)"] = solder_paste_thickness
                            current_data.loc[0, "Weight of solder paste for 100%(g)"] = weight_of_solder_paste_for_100percentage_wt_value
                            current_data.loc[0, "Top Weight of solder paste for Weight Estimate (g)"] = top_weight_of_solder_paste_for_wt_estimate_value
                            current_data.loc[0, "Top Side Cost Per Board($)"] = top_side_cost_per_board_value
                            current_data.loc[0, "Bottom Weight Estimate %"] = bot_weight_estimate_percentage
                            current_data.loc[0, "Bottom Solder Paste Wastage %"] = bot_sp_wastage_percentage
                            current_data.loc[0, "Bottom Weight of solder paste for Weight Estimate (g)"] = bot_weight_of_solder_paste_for_wt_estimate_value
                            current_data.loc[0, "Bottom Side Cost Per Board($)"] = bot_side_cost_per_board_value
                            current_data.loc[0, "Flux Wastage %"] = flux_wastage_percentage
                            current_data.loc[0, "Flux Cost($/ml)"] = flux_cost
                            current_data.loc[0, "Board Area(mm^2)"] = flux_board_area_value
                            current_data.loc[0, "Flux Spread Area(mm^2)"] = flux_spread_area_value
                            ...
                            # current_data.loc[0, "MOH %"] = 0.0
                            # current_data.loc[0, "FOH %"] = 0.0
                            # current_data.loc[0, "Profit on RM %"] = 0.0
                            # current_data.loc[0, "Profit on VA %"] = 0.0
                            # current_data.loc[0, "R&D %"] = 0.0
                            # current_data.loc[0, "Warranty %"] = 0.0
                            # current_data.loc[0, "SG&A %"] = 0.0
